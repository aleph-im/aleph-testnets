...
#
# CRN /control/allocations auth uses EIP-191 signatures (aleph-vm#945): the
# scheduler signs requests with the key generated by local-up.sh --env, and
# each CRN authorizes the derived address (.local/crn-signer.env).
#
...
install_crn() {
    ...
    # EIP-191 allocation auth: authorize the scheduler's signer address on
    # each CRN. The keypair is generated by local-up.sh --env.
    local signer_addr
    ...
    echo "==> CRN allocation signer: $signer_addr"
    ...
    for idx in $(seq 0 $((CRN_COUNT - 1))); do
        ...
        # AUTHORIZED_ALLOCATION_SIGNERS is a pydantic list field parsed as JSON,
        # so the inner double quotes are required. The single quotes keep systemd
        # (EnvironmentFile in aleph-vm-supervisor.service) from stripping them.
        cat > "$env_file" <<EOF
ALEPH_VM_SUPERVISOR_HOST=0.0.0.0
ALEPH_VM_DOMAIN_NAME=$ip
ALEPH_VM_API_SERVER=$CCN_URL
ALEPH_VM_OWNER_ADDRESS=$CRN_OWNER_ADDR
ALEPH_VM_AUTHORIZED_ALLOCATION_SIGNERS='["$signer_addr"]'
